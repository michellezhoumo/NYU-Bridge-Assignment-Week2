...
template <class T> //Return the height of a node in the tree
...
template <class T> //Overloaded in RBT, the overloaded func calls prettyPrint(0)
...
template <class T> //overloads the initial swap color function that takes a int of 0 or 1 and return the flip color (line 22)
...
public:
    RBT() : root(nullptr) {} //RBT default constructor with nothing inside

    void insert(const T &); //insert data of type T
    void insert(const T &, RBTNode<T> *&point, RBTNode<T> *parent);
    //point = 1. temp pointer to move around to search for insert position
    //point = 2. the new Node to be inserted
    //parent = leaf node? inserted node's parent
    void prettyPrint() const { root->prettyPrint(0); } //overloading the function in RBTnode.

    int height() const { return root->height(); }
};

template <class T> //RL Config
void RBT<T>::doubleCCR(RBTNode<T> *&point) {
    singleCR(point->right);
    singleCCR(point);
}

template <class T> //LR Config
void RBT<T>::doubleCR(RBTNode<T> *&point) {
    singleCCR(point->left);
    singleCR(point);
}

template <class T> //LL Config
...
//assuming RBT is not empty? Where's the check for empty RBT and insert of black root node
template <class T>
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        if(curr_node->parent == nullptr) {
            swapColor(curr_node); //set the root to be a blacknode
        }

            //identify a red-red conflict and starting to move curr_node up the red black tree toward the root
        while (curr_node->color == RED && curr_node->parent->color == RED) {
            temp = curr_node;
            curr_node = curr_node->parent;
            //check if it is the left child
            if (curr_node->parent->left== curr_node) {
                //if uncle node is black or null
                if (curr_node->parent->right == nullptr || curr_node->parent->right->color == BLACK) {
                    if (curr_node->left == temp) {
                        //single CR needed - config LL
                        singleCR(curr_node->parent);
                    } else {
                        //double CR needed - config LR
                        doubleCCR(curr_node-> parent);
                    }
                        ...
                        swapColor(curr_node);
                        swapColor(curr_node->parent->right);
                        if (curr_node->parent->parent != nullptr) {
                            swapColor(curr_node->parent);
                            cout << curr_node ->data << " " << curr_node->color << endl;
                            cout << curr_node -> parent ->data << " " << curr_node -> parent ->color << endl;
                    ...
                    if (curr_node->parent->left == nullptr || curr_node->parent->left->color == BLACK) {
                        if (curr_node->right == temp) {
                            //single CCR needed - RR
                            singleCCR(curr_node -> parent);
                        } else {
                            //double CCR needed - RL
                            doubleCR(curr_node -> parent);
                        }
                        swapColor(curr_node->left);
                        swapColor(curr_node);
                    } else {
                        swapColor(curr_node);
                        swapColor(curr_node->parent->left);
                        if (curr_node->parent->parent != nullptr) {
                            swapColor(curr_node->parent);
                            curr_node = curr_node->parent;
                        }
